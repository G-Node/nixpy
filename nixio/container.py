...
class LinkContainer(Container):
    """
    LinkContainer is a counterpart to the Container class for linked objects.
    Link containers differ from regular containers in the following ways:
    - Linked objects are not created by the parent object but are appended to
    the LinkContainer explicitly. Therefore, LinkContainer provides append()
    and extend() methods.
    - Linked objects are indexed by id rather than name.
